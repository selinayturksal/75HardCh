...
export default function AcademicScreen() {
  ...
  const { todayDone, markToday, getDayCount, getProgressPercent } = useProgress('academic');
  ...
  const handleSave = async () => {
    if (!studyTopic || !studyDuration) {
      Alert.alert(t.fillAll);
      return;
    }
    await markToday({ studyTopic, studyDuration, notes });
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      <View style={[styles.header, { backgroundColor: COLOR }]}>
        ...
        <Text style={styles.headerTitle}>🎓 {t.academic}</Text>
        <View style={{ width: 40 }} />
      </View>
      ...
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        <View style={[styles.progressCard, { backgroundColor: theme.card }]}>
          ...
          <View style={styles.starContainer}>
            <View style={[styles.starOuter, { borderColor: COLOR }]}>
              <View style={[styles.starFill, { height: `${percent}%`, backgroundColor: COLOR + 'AA' }]} />
              <Text style={[styles.starText, { color: COLOR }]}>{dayCount}</Text>
            </View>
        ...
        {!todayDone ? (
          <View style={[styles.formCard, { backgroundColor: theme.card }]}>
            ...
            <Text style={[styles.label, { color: theme.textLight }]}>{t.studyTopic}</Text>
            <TextInput
            ...
            <Text style={[styles.label, { color: theme.textLight }]}>{t.studyDuration}</Text>
            <TextInput
              style={[styles.input, { backgroundColor: theme.background, color: theme.text, borderColor: theme.border }]}
              placeholder="60"
              placeholderTextColor={theme.textLight}
