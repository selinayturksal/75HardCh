...
export default function SportScreen() {
  ...
  const { completedDays, todayDone, markToday, getDayCount, getProgressPercent } = useProgress('sport');
  ...
  const handleSave = async () => {
    ...
    await markToday({
      activity: selectedActivity === 'custom' ? customActivity : selectedActivity,
      duration,
      calories,
    });
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      <View style={[styles.header, { backgroundColor: COLOR }]}>
        ...
        <Text style={styles.headerTitle}>🏋️ {t.sport}</Text>
        <View style={{ width: 40 }} />
      </View>
      ...
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        {!todayDone ? (
          <View style={[styles.formCard, { backgroundColor: theme.card }]}>
            ...
            {/* ACTIVITY SELECT */}
            <Text style={[styles.label, { color: theme.textLight }]}>{t.activity}</Text>
            <View style={styles.activityGrid}>
              {ACTIVITIES.map(act => {
                const label = t[act.id as keyof typeof t] as string;
            ...
            <Text style={[styles.label, { color: theme.textLight }]}>{t.calories}</Text>
            <TextInput
              style={[styles.input, { backgroundColor: theme.background, color: theme.text, borderColor: theme.border }]}
              placeholder="300"
              placeholderTextColor={theme.textLight}
